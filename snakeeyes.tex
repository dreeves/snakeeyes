...
\usepackage{graphicx}
\usepackage{hyperref}
\hypersetup{colorlinks=true, allcolors=blue, urlcolor=blue}
\usepackage{emoji}
...
% not using this yet:
\newcommand{\skull}{\ensuremath{\text{\emoji{skull}}}}
...
Controversy, that I think turns out to be moot unless we go down the hyperreal rabbit hole, is which of the following that sentence means:

(a) We keep going until one of those groups is devoured by snakes, then the game stops.
(b) Assume that snake eyes is rolled in a finite number of rolls and stop when that happens.

The subtle difference is that I didn't mean to stipulate that snake eyes is rolled in a finite number of rolls, just that there's a probability 1 of that happening.

~~

Clarifications in the market description as of 2023-10-30:

1. The game is not adversarial; the dice rolls are independent and fair.
